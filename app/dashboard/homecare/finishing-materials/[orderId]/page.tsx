...
import { FinishingOrderDetail } from "@/components/finishing-order-detail";

export default async function FinishingMaterialOrderPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  const { data: order } = await supabase.from("finishing_material_orders").select("*").eq("id", orderId).single();
  if (!order) notFound();
  ...
  return (
    <main className="relative min-h-screen overflow-hidden px-4 py-8">
      ...
      <div className="relative z-10 mx-auto max-w-md">
        ...
        <FinishingOrderDetail order={order} items={items ?? []} isAdmin={isAdmin} />
      </div>
    </main>
  );
}
