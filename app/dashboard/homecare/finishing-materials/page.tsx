...
import { FinishingMaterialCatalog } from "@/components/finishing-material-catalog";
...
export default async function FinishingMaterialsPage() {
  ...
  const ordersQuery = supabase
    .from("finishing_material_orders")
  ...
  return (
    <main className="relative min-h-screen overflow-hidden px-4 py-8">
      ...
      <div className="relative z-10 mx-auto max-w-md">
        ...
        <h1 className="mb-1 mt-3 text-lg font-bold">Finishing Material Supply</h1>
        <p className="mb-5 text-sm text-[var(--adrith-dim-2)]">
          Paints, doors & windows, glass, tiles, stainless steel, plumbing & electrical fixtures. Enter what you need — payment happens after we confirm availability with the vendor.
        </p>

        <Link
          href="/dashboard/homecare/finishing-materials/new"
          ...
          {orders && orders.length > 0 ? (
            <div className="flex flex-col gap-2">
              {orders.map((o) => (
                <Link
                  key={o.id}
                  href={`/dashboard/homecare/finishing-materials/${o.id}`}
        ...
        <div className="border-t border-white/10 pt-5">
          ...
          <FinishingMaterialCatalog materials={materials ?? []} isAdmin={isAdmin} />
        </div>
      </div>
    </main>
  );
