...
export function FinishingNewOrderForm({ materials, userId }: { materials: Material[]; userId: string }) {
  ...
  async function submit() {
    ...
    const { error: itemsError } = await supabase.from("finishing_material_order_items").insert(
      lineItems.map((row) => ({
        order_id: order.id,
