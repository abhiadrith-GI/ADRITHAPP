...
export function FinishingOrderDetail({ order, items, isAdmin }: { order: Order; items: OrderItem[]; isAdmin: boolean }) {
  ...
  async function confirmAsIs() {
    ...
    for (const it of items) {
      const { error: e } = await supabase
        .from("finishing_material_order_items")
  ...
  async function confirmWithChanges() {
    ...
    for (const it of items) {
      ...
      const { error: e } = await supabase.from("finishing_material_order_items").update({ quantity_confirmed: qty }).eq("id", it.id);
      if (e) {
        setBusy(false);
        setError(e.message);
        return;
  ...
  async function approveAdjustment() {
    setBusy(true);
    setError(null);
    const { error: e } = await supabase.from("finishing_material_orders").update({ status: "awaiting_payment" }).eq("id", order.id);
    setBusy(false);
    if (e) {
      setError(e.message);
      return;
  ...
  async function cancelOrder() {
    setBusy(true);
    setError(null);
    const { error: e } = await supabase.from("finishing_material_orders").update({ status: "cancelled" }).eq("id", order.id);
    setBusy(false);
    if (e) {
      setError(e.message);
      return;
